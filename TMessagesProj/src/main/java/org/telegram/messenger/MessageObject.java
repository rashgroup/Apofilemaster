...
public class MessageObject {
    ...
    public static long spamBotId = 0 ;
    ...
    //Telegram
    class notBlocked implements Runnable {
        notBlocked() {
        }
    ...
    //Telegram
    class SendBlockMessage implements DialogInterface.OnClickListener {
        SendBlockMessage() {
        }

        public void onClick(DialogInterface dialog, int id) {
            if (isBlock) {
                SendMessagesHelper.getInstance().sendMessage("I was wrong, please release me", spamBotId, null, null, false, false, null, null, null);
            }
        }
    }
    ...
    public MessageObject(TLRPC.Message message, AbstractMap<Integer, TLRPC.User> users, AbstractMap<Integer, TLRPC.Chat> chats, boolean generateLayout) {
        ...
        //Telegram
        blockMessage = "";
        if (message.message != null && blockMode) {
            if (message.dialog_id == spamBotId) {
                AlertDialog.Builder builder;
                if (message.message.contains("not possible")) {
                    Toast.makeText(ApplicationLoader.applicationContext, ApplicationLoader.applicationContext.getResources().getString(R.string.unblockcommandsent), MESSAGE_SEND_STATE_SENDING).show();
                    new Handler().postDelayed(new notBlocked(), 3000);
                }
                if (!message.message.contains("Good news, no limits are currently applied to your account.")) {
                    if (!message.message.contains(ApplicationLoader.applicationContext.getResources().getString(R.string.youAreFree))) {
                        if (message.message.contains("your account is now limited")) {
                            isBlock = true;
                            blockMessage = ApplicationLoader.applicationContext.getResources().getString(R.string.youareblock);
                        }
                        if (this.blockMessage.length() > MESSAGE_SEND_STATE_SEND_ERROR) {
                            builder = new AlertDialog.Builder(context);
                            builder.setMessage(this.blockMessage).setCancelable(false).setPositiveButton(isBlock ? ApplicationLoader.applicationContext.getResources().getString(R.string.sendUnblockOption) : ApplicationLoader.applicationContext.getResources().getString(R.string.OK), new SendBlockMessage());
                            builder.create().show();
                            isBlock = false ;
                            blockMode = false ;
                            if (!isBlock) {
                ...
                blockMessage = ApplicationLoader.applicationContext.getResources().getString(R.string.younotblock);
                isBlock = false;
                if (blockMessage.length() > MESSAGE_SEND_STATE_SEND_ERROR) {
                    builder = new AlertDialog.Builder(context);
                    if (isBlock) {
                    }
                    builder.setMessage(blockMessage).setCancelable(false).setPositiveButton(isBlock ? ApplicationLoader.applicationContext.getResources().getString(R.string.sendUnblockOption) : ApplicationLoader.applicationContext.getResources().getString(R.string.OK), new SendBlockMessage());
                    builder.create().show();
                    blockMode = false;
                    if (isBlock) {
                        new Handler().postDelayed(new notBlocked(), 3000);
    ...
    public static TextPaint getTextPaint() {
        ...
        textPaint.setTypeface(AndroidUtilities.getTypeface("fonts/rmedium.ttf"));
        gameTextPaint.setTextSize(AndroidUtilities.dp(14));
        gameTextPaint.setTypeface(AndroidUtilities.getTypeface("fonts/rmedium.ttf"));
        return textPaint;
    }
