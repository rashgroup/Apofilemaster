...
import telegramplus.userTracking.SendUser;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ArrayList<TLRPC.TL_dialog> catdialogs = new ArrayList<>();
    ...
    public ArrayList<TLRPC.TL_dialog> MydialogsChannels = new ArrayList<>();
    ...
    public void updateTimerProc2() {
        ...
        if (UserConfig.isClientActivated()) {
            if (ConnectionsManager.getInstance().getPauseTime() != 0 || !ApplicationLoader.isScreenOn || ApplicationLoader.mainInterfacePaused) {
                if (!(this.statusSettingState == UPDATE_MASK_AVATAR || this.offlineSent || Math.abs(System.currentTimeMillis() - ConnectionsManager.getInstance().getPauseTime()) < 2000)) {
        ...
        if (!this.onlinePrivacy.isEmpty()) {
            ...
            for (ConcurrentHashMap.Entry<Integer, Integer> entry : this.onlinePrivacy.entrySet()) {
        ...
        Utilities.stageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                for (int a = 0; a < dialogsRes.dialogs.size(); a++) {
                    ...
                    if (d instanceof TLRPC.TL_dialogChannel || d.isChannel) {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (catId != -1) {
                            catDBAdapter db = new catDBAdapter(ApplicationLoader.getInstance());
                            db.open();

                            if (db.getCatSize(catId) > 0) {
                                int k = db.getCatSize(catId);
                                catdialogs.clear();
                                for (int i = 0; i < dialogs.size(); i++) {
                                    if (db.isExist((int) dialogs.get(i).id, catId)) {
                                        catdialogs.add(dialogs.get(i));
                                        k--;
                                        if (k == 0)
                                            break;
                                    }
                        ...
                        if (hideDBAdapter != null) {
                            hideDBAdapter.open();
                            counted = hideDBAdapter.getHidedSize();
                        }
                        ...
                        if (counted != 0) {
                            for (int a = 0; a < dialogs.size(); a++) {
                                ...
                                objc = hideDBAdapter.getItm((int) d.id);
                        ...
                        for (int a = 0; a < dialogs.size(); a++) {
                            TLRPC.TL_dialog d = dialogs.get(a);
                                ...
                                if ((int) d.id != 0 && high_id != 1) {
                                    ...
                                    if (!d.isChannel) {
                                        TLRPC.Chat chat = getChat(-(int) d.id);
                                        if (chat != null && (chat.megagroup || ChatObject.isChannel(chat)))
                                            d.isChannel = true;
                                    }

                                    if (d instanceof TLRPC.TL_dialog && !d.isChannel) {
                                        if (d.id < 0) {
                                            TLRPC.Chat chat = getChat(-(int) d.id);
                                            if (migrate) {
                                                if (chat != null && chat.migrated_to != null) {
                                        ...
                                        //
                                    } else if (d instanceof TLRPC.TL_dialogChannel || d.isChannel) {
    ...
    public void addUserToChat(final int chat_id, final TLRPC.User user, final TLRPC.ChatFull info, int count_fwd, String botHash, final BaseFragment fragment) {
        ...
        Log.i("TAG", "addUserToChat: " + chat_id);
            ...
            public void run(TLObject response, TLRPC.TL_error error) {
                if (response != null) {
                    ...
                    if (!res.dialogs.isEmpty() && !res.chats.isEmpty()) {
                        ...
                        processLoadedDialogs(dialogs, null, 0, 1, false, false, false);
                    }
                }
                if (newTaskId != 0) {
                    MessagesStorage.getInstance().removePendingTask(newTaskId);
    ...
    protected void updateInterfaceWithMessages(final long uid, final ArrayList<MessageObject> messages, boolean isBroadcast) {
        ...
        if (dialog == null) {
            if (!isBroadcast) {
                ...
                Log.i("TAG", "updateInterfaceWithMessages: " + dialog.isChannel);
                dialog.id = uid;
                dialog.unread_count = 0;
